...
public class ImageUtil {
	...
	public static void removeUploadImage(HttpServletRequest request, String filename) {
		...
		synchronized (lock) {
			if (imageList.contains(filename))
				...
				logger.error("Image file not exist. filename = " + filename);// 当Controller尝试保存一个已经不存在的图片时返回错误
				// throw new
				// RuntimeException("Image file not exist.");//当Controller尝试保存一个已经不存在的图片时返回错误
		}
	}

	/**
	 * 从用户session的图片文件列表和全局图片列表中清除文件记录(文件不存在不抛出异常)
	 * 
	 * @param request
	 * @param filename
	 */
	public static void tryRemoveUploadImage(HttpServletRequest request, String filename) {
		@SuppressWarnings("unchecked")
		List<String> list = (List<String>) request.getSession().getAttribute(CommonsConstant.SESSION_IMAGES);
		if (CollectionUtils.isNotEmpty(list)) {
			list.remove(filename);
	...
	public static void tryRemoveUploadImageByFilteringContent(final HttpServletRequest request, final String content) {
		if (StringUtils.isEmpty(content)) {
			return;
		}
		final Pattern pattern = Pattern.compile("src=\"[a-z0-9A-Z/\\-_:\\.]+\"");
		...
		while (matcher.find()) {
			String src = content.substring(matcher.start(), matcher.end());
			String fileName = src.substring(src.lastIndexOf("/") + 1, src.length() - 1);
			ImageUtil.tryRemoveUploadImage(request, fileName);
		}
	}
	...
	/*	File file = new File(buildFile("C:", UUID.randomUUID().toString() + ".jpg"));
		...
		System.out.println(ImageUtil.buildFile("", "2393422d-a941-4b2d-bc7a-83707a890b6d.doc"));

     ...
     <div class="container w1200 marginauto bkgNone clearfix">
       <div class="content">
	       <p class="uploadContainer">
	  	     <span class="fr">
			   <a href="${root}/teacherTest/toCreateExam.html" target="_blank" class="btn btnUpload mr20" >组建试卷</a>
			   <a href="javascript:uploadExam();" class="btn btnUpload" >导入试卷</a>
		    </span>
		   </p>
	...
	<script type="text/javascript">
    	...
    	$(document).on('click', '.delexam', function (event) {
    		...
    		Win.confirm({id:"delExamId",html:"您确定要删除该试卷吗？"},function(){
    			$.post("${root}/teacherTest/delExam.do",{"examId":$elm.attr("eid")},function(data){
    				console.log(data);
    				if(data == 1){
    					Win.alert("删除成功！");
    					getExamData(user_schoolId);
    	...
    	$(document).on('click', '.bzexam', function (event) {
    		...
    		Win.open({
				...
				width : 520,
				height : 570,
				url : "${root}/teacherTest/selectStudentsPre.do?examId="+$elm.attr("eid")
			});
    	})
		...
		var splitPage ;
        var url = "${root}/teacherTest/getExamList.do";
    	  ...
    	  if(total || total != 0){
    		if(data){
    			...
    			for (var i = 0; i < data.length; i++){
    				...
    				taskHtml += '<h4 class="examTitle"><a href="${root}/teacherTest/viewExam/exam/'+task.examId+'.html" target="_blank">'+task.title+'</a></h4>';
    				...
    				taskHtml += '<div class="examDesc">更新时间：<span>'+task.createTime+'</span>'
                           ...
                           +'</div>';
                       taskHtml += '<a href="${root}/teacherTest/editExam/exam/'+task.examId+'.html" target="_black" class="examState editexam  threeItem" id='+task.status+' eid='+task.examId+'>编辑</a>';
      ...
      function uploadExam(){
    		 Win.open({id:'uploadExam',width:500,height:520,title:"导入试卷",url:"${root}/questionLib/uploadQuestionPage.do?type=exam"});
    	}

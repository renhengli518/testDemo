...
<body class="mainIndex">
       ...
       <div class="content">
          <div class="content-in">
              <div class="breadCrumb">
                <a href="javascript:;">班级测试</a><span class="nextlevel">></span><span class="currentLevel">统计 </span>
              </div>
			  <!-- 数据列表 -->
			    	 ...
			    	 <li>
			    	  <label class="text gn-labelText"><b>学科：</b></label>
			    	   <span class="cont gn-sortBox  clearfix">
			    	      <span class="mr30">${examTaskView.subjectName}</span>
			    	             试卷类型：<span class="mr30">${examTaskView.examTypeName}</span>测试人数：<span class="mr30"><span class="red">${examTaskView.finishedCount}</span>/${examTaskView.assignedCount}</span>
    ...
    <script  type="text/javascript">
      ...
      $(document).ready(function(){
    	 ...
    	 $("#exportData").click(function(){
    		...
    		$.post(url, obj, function(data){
    			if(data.result){
    				window.location.href = "${root}/classTest/exportStatisticsUrl.do";
    			}else{
    				Win.alert("导出失败！");
    			}
    		});
      ...
      function bindStatistics(){
    	var examTaskId = '${examTaskView.examTaskId}';
    	var url = "${root}/classTest/getExamStatisticsByClass.do";
    	$.post(url, {examTaskId:examTaskId}, function(data){
    		if(data != null && data.length > 0){
                ...
                for(var i = 0; i < data.length; i++){
    				examStatis = data[i];
                	html += '<tr>';
    			    html += '<td>'+examStatis.className+'</td>';

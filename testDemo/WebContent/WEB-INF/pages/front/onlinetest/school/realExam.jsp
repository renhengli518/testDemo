...
<body class="mainIndex">
	...
	<div class="container w1200 marginauto posRelative bkgNone clearfix">
		<p class="uploadContainer">
			<span class="fr"> 
				<a href="${root}/schoolTest/createRealExam.html" class="btn btnUpload" id="addRealExam">创建试卷</a>
				<a href="javascript:uploadRealExam();" class="btn btnUpload mr20" id="importRealExam">导入试卷</a> 
			</span>
		</p>
		<div class="content">
	...
	function searchQuestions() {
		var url  = '${root}/schoolTest/searchRealExamList.do';
	...
	function toEditRealExam(obj){
		...
		window.open("${root}/schoolTest/editRealExam.html?examId="+examId);
	}
	...
	function toDeleteRealExam(obj){
		...
		Win.confirm("确认要删除该真题试卷吗？", function(){
	 		if(examId==null || examId==""){
				...
				$.get("${root}/schoolTest/deleteRealExamById.do",{"examId":examId},function(data){
					if(data&&data.result){
						$(obj).parents(".borderBox").remove();
						Win.alert("删除真题试卷成功！");
						//searchQuestions();
					}else{
		...
		if(total || total != 0){
    		if(data){
    			...
    			for (var i = 0; i < data.length; i++){
    				...
    				taskHtml += '<h4 class="examTitle"><a href="${root}/schoolTest/viewExam/'+task.examId+'.html" target="_blank">'+task.title+'</a></h4>';
    				...
    				taskHtml += '<div class="examDesc">'
                           ...
                           +'</div>';
                       taskHtml += '<a href="javascript:;" class="examState  twoItem"  id='+task.examId+' onclick="toEditRealExam(this)">编辑</a>';
                       taskHtml += '<a href="javascript:;" class="examState  twoItem"  id='+task.examId+' onclick="toDeleteRealExam(this)">删除</a>';
                    taskHtml += '</div></div>';  
    			}
    			...
    			$("#pageBody").html('<div class="borderBox pl40 testWithLabel"  align="center">'+noRecord+'</div>');
    		}
    	}else{
    		$("#pageBody").html('<div class="borderBox pl40 testWithLabel"  align="center">'+noRecord+'</div>');
    	}
    ...
    function uploadRealExam (){
		 Win.open({id:'uploadQuestion',width:500,height:520,title:"导入真题试卷",url:"${root}/questionLib/uploadQuestionPage.do?type=realexam"});
	}
    </script>
</body>
</html>
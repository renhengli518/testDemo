       ...
       <div class="content">
          <div class="content-in">
              <div class="breadCrumb">
                <a href="javascript:;">班级测试</a><span class="nextlevel">></span><a href="javascript:;">统计</a><span class="nextlevel">></span><span class="currentLevel">查看答题</span>
              </div>
			  <!-- 数据列表 -->
			             ...
			             <c:if test="${q.type == 'FILL_IN_BLANK'}">
					       ...
					       <c:if test = "${not empty q.fillInAnswers}">
					        ...
					        <c:if test="${!empty q.myAnswer}">
					          ...
					          <c:if test="${q.difficulty == 'VERYEASY'}">
								      较易
						      </c:if>
						      <c:if test="${q.difficulty == 'NORMAL'}">
								      一般
						      </c:if>
						      <c:if test="${q.difficulty == 'VERYHARD'}">
								       较难
						      </c:if>
						      <c:if test="${q.difficulty == 'HARD'}">
								      困难
						     </c:if>
					       </span>
					                        分值：  <span class="mr40 red">${q.score}</span>分   

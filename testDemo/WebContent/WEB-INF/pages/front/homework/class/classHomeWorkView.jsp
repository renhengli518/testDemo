									...
									<div class="borderBox pd20">
										<div class="quesContent verticalMiddle">
											...
											<c:if test="${ques.type == 'FILL_IN_BLANK'}">
												...
												<span class="mr40">
												 <c:if
														test="${!empty ques.fillInQuesAnswer}">
														<table class="anyTable">
															<thead>
																<tr>
																	<th>
																		<div class="specialTH">
																			<span class="rightTop">答案容错</span> <span
																				class="leftBottom">填空</span>
																		</div>
																	</th>
															...
															<tbody>
																<c:forEach var="fillInAnswer"
																	items="${ques.fillInQuesAnswer}" varStatus="fs">
																	<tr>
																		<td>第${fs.index +1}空</td>
																		<td class="edit-pop-btn"><div
																				 class="answerCon">${fillInAnswer.answerGrp1}</div></td>
																		<td class="edit-pop-btn"><div
																				 class="answerCon">${fillInAnswer.answerGrp2}</div></td>
																		<td class="edit-pop-btn"><div
																				 class="answerCon">${fillInAnswer.answerGrp3}</div></td>
																		<td class="edit-pop-btn"><div
																				 class="answerCon">${fillInAnswer.answerGrp4}</div></td>
																	</tr>
																</c:forEach>
															</tbody>
														</table>
											...
											<div class="teaComments">
												<div style="display: block" class="borderBox comments">
													<p class="commentsKinds">
														<a href="javascript:;" class="active" pid="${ques.workQuestionId}">习题解析</a> 
														<a href="javascript:;" pid="${ques.workQuestionId}">知识点</a>
													</p>
													<div style="display: block"
														class="pd10 commentContent showResolveView">
														${ques.resolve}</div>
								...
								<ul class="commonUL">
									<li><label class="text  fontBold">附件作业：</label> 
										...
										<fmt:formatNumber type="number" value="${file.docSize/1024}" maxFractionDigits="2"/>K
														...
														<a class="ml30 lightBlue"
															href="${root}/images/${file.docPath}">下载</a>
														<br />
											</c:forEach>
											</span>								
									</li>

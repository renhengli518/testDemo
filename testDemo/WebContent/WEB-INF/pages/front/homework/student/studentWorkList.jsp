...
function listHomeWork() { 
	...
	var config = {
			node:$id("pager"),
			url:"${root}/studentWork/getStudentWorkList.do?r="+(+new Date),
			count :20,
			type :"post",
...
function homeWorkListResult(data,total){
	...
	if(total && data){
		...
		for(var i = 0,j = data.length;i<j;i++){
			...
			console.log(homeWorkId);
				...
				html+='<h4 class="examTitle"><a href="${root}/studentWork/stuHomeWorkView/'+homeWorkId+'/'+"${userId}"+'/'+homeWork.status+'.do"' +'target="_blank" >'+homeWork.workTitle+'</a></h4>';
				...
				html+='答题数：<span><span class="red fontNormal">'+homeWork.answerCount+'</span>/'+homeWork.queCount+'</span>';
				html+='状态：<span>'+status+'</span>';
				html+='提交时间：<span>'+submitTime+'</span>';
				html+='</div>';
		if(homeWork.status=='PROGRESS'){
				
				html+='<a href="javascript:;" class="examState threeItem answer">答作业</a>';
				
			}
		if(homeWork.status =='CHECKED'){
			html+='<a href="${root}/studentWork/toStudentReadOverWorkView/'+homeWorkId+'.do"'+'class="examState threeItem">'+'查看批阅</a>';
		}
				html+='</div>';
				$("#pageBody").html(html);
			}
	...
	$("#pageBody").on("click",".answer",function(){
		var homeworkId = $(this).parents(".borderBox").attr("homeworkId");
		location.href="${root}/homework/answerHomeworkPre.html?homeworkId="+homeworkId;
	});
</script>
</html>
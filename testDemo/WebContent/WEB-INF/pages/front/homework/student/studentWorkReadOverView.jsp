...
<style type="text/css">
#showInfo {width:250px;height:100px;border:1px solid #f00;}
</style>
...
$(function(){
	$(".checkAnswerWrap").on("click",".commentsKinds a",function(){
		...
		if($this.hasClass("showOne")){
			...
			var html="";
			if(text=="习题解析" ){
					$.post("${root}/teacherWork/getResolve.do",{"queId":queId},function(data){
						var html = data;
						if(data!="" || data!=null){
					...
					var html="";
					var knowLedge="";
					$.post("${root}/teacherWork/getQuestionKnowLedge.do",{"queId":queId},function(data){
						if(data.length>0){
							for(var i=0;i<data.length;i++){
								if(data[i].knowLedgeName==null){
									knowLedge="";
								}else{
									knowLedge=data[i].knowLedgeName;
								}
								html+="<span>"+knowLedge+"</span>";
							}
	...
	<div class="container clearfix w1200 bkgNone marginauto">
		<div class="content">
			<div class="content-in">
				...
				<c:if test="${myWorkRead =='queryRead' }">
					<a href="javascript:history.go(-1);">我的作业</a><span class="nextlevel">></span><span class="currentLevel">查看批阅 </span>
				</c:if>
				<c:if test="${studentRead =='student' }">
					<a href="${root}/studentWork/myReadOver.do">作业</a><span class="nextlevel">></span><a href="javascript:history.go(-1);">我的批阅</a><span class="nextlevel">></span><span class="currentLevel">查看批阅 </span>
				</c:if>
				</div>
			<div class="borderBox gn-bgWhite pd20 pb0">
				<c:if test="${!empty user}">
					<h3 class="MarginTitle" id="workTitle">答题人：
					<span class="coffeeColor">${userName}</span>
					&nbsp;&nbsp;&nbsp;&nbsp;
					批阅人：<span class="coffeeColor">${user.userName}</span>
					</h3> 
				</c:if>
				<c:if test="${empty user}">
					<h3 class="MarginTitle BorderBottomNone" id="workTitle">答题人：<span class="coffeeColor">${userName}</span></h3> 
				</c:if>
			</div>
				...
				<div class="checkAnswerWrap">
					<div class="testCon checkAnsweContent">
						<c:if test="${!empty homeWorkQuestionInfo.questionInfo}">
							<c:forEach var="ques" items="${homeWorkQuestionInfo.questionInfo}" varStatus="s">
							<%@include file="../../../common/workBodytop.jsp"%>
								...
								<div class="teaComments">
									<div style="display:block" class="borderBox comments">
										<p class="commentsKinds">
											<c:if test="${homeWorkQuestionInfo.readOverType != 'TEACHER'}">
												<a href="javascript:;" class="active" pid="${ques.workQuestionId}">学生点评</a>
											</c:if>
											<c:if test="${homeWorkQuestionInfo.readOverType == 'TEACHER'}">
												<a href="javascript:;" class="active" pid="${ques.workQuestionId}">老师点评</a>
											</c:if>
											<c:if test="${homeWorkQuestionInfo.readOverType == 'TEACHER'}">
									...
									<div style="display:block" class="pd10 commentContent" id="showCommentView">
										<c:if test="${homeWorkQuestionInfo.readOverType != 'TEACHER'}">
											<c:choose>
												<c:when test="${not empty ques.comment && ques.comment ne ' '}"><div class="imageContent">${ques.comment}</div></c:when>
												<c:otherwise><div>学生还没有点评！</div></c:otherwise>
											</c:choose>
										</c:if>
										<c:if test="${homeWorkQuestionInfo.readOverType == 'TEACHER'}">
										<c:choose>
											<c:when test="${not empty ques.comment && ques.comment ne ' '}"><div class="imageContent">${ques.comment}</div></c:when>
											<c:otherwise><div>老师还没有点评！</div></c:otherwise>
										</c:choose>
										</c:if>
									</div>
										<div class="pd10 commentContent showResolveView">
										</div>
										<div class="pd10 commentContent showKnowLedgeView" >
										</div>
								<c:if test="${homeWorkQuestionInfo.readOverType == 'TEACHER'}">
									<c:if test="${! empty ques.resolveVideo}">
										<p class="playVideoP"> <button class="btn playVideo" onclick="playVideo('${ques.resolveVideo}','${ques.workQuestionId}')">点播解析视频</button></p>
									</c:if>
								</c:if>
								</div>
							</div>
							...
							<h3 class="titleOfCheckAnswer clearfix">文本作业</h3>
								<ul class="commonUL">
									<li><label class="text  fontBold">文本作业：</label> 
										<span class="cont"> ${homeWorkQuestionInfo.textQueContent}
										</span>
									</li>
								</ul>
								<ul class="commonUL">
									<li>
										...
										<span class="cont"> ${homeWorkQuestionInfo.textQueAnswer} </span>
									</li>
								</ul>
									<div class="teaComments">
										<div style="display: block" class="borderBox comments">

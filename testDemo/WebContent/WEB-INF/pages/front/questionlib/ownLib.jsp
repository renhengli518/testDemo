...
<body class="mainIndex">
	...
	function searchQuestions() {
		var url  = '${root}/questionLib/searchOwnList.do';
	...
	function deleteQuestion(obj){
		var queQuestionId = $(obj).prop("id");
		var questionType = $(obj).parent().next().find(".active").attr("data-type");
		var confirmInfo = "";
		if(questionType == "MOTHER"){
			confirmInfo = "删除母题，所有子题也会被删除，确认要删除该习题吗？";
		} else {
			confirmInfo = "确认删除子题？";
		}
		Win.confirm(confirmInfo, function(){
	 		if(queQuestionId==null || queQuestionId==""){
				Win.alert("删除习题失败！");
			} else {
				$.get("${root}/questionLib/deleteOwnQuestion.do",{"queQuestionId":queQuestionId},function(data){
					if(data&&data.result){
						$(obj).parents("#quesContainer").remove();
						$("#totalCnt").text($("#totalCnt").text()-1);
						Win.alert("删除习题成功！");
						//searchQuestions();
					}else{
	...
	function formQuestionList(data,totalCnt){
		...
		if(len>0){
			for(var i = 0; i< len; i++){
				...
				if(userType=="TEACHER"){
					if(qstObj.usableType=="PUBLIC" || qstObj.usableType=="SCHOOL"){//老师共享的才有审核标记
				...
				if(qstObj.usableType=="PUBLIC" || qstObj.usableType=="SCHOOL"){
					qstHtml += '<a herf="javascript:;" class="gray">共享</a><b class="fontNormal c888"> | </b>';
				}else if(qstObj.usableType=="PRIVATE"){
					qstHtml += '<a herf="javascript:;" class="gray">私有</a><b class="fontNormal c888"> | </b>';
				}
				qstHtml += ' 	<a style="cursor:pointer;" herf="javascript:;" id="'+qstObj.queQuestionId+'" onclick="editQuestion(this)" class="skyBlue">编辑</a>';
				qstHtml += ' 	<b class="fontNormal c888"> | </b>';
				qstHtml += ' 	<a style="cursor:pointer;" herf="javascript:;" id="'+qstObj.queQuestionId+'"  onclick="deleteQuestion(this)" class="skyBlue deleteQuestion">删除</a>';
				qstHtml += '</p>';
				qstHtml +='<div class="quesBro">';
				qstHtml +='		<a href="javascript:;" class="active" data-type="'+qstObj.relationalIndicator+'" onclick="switchQues(this)" id="'+qstObj.queQuestionId+'">母题</a>';
				...
				if(childrenQuestionList.length>0){
					...
					for(var j=0;j<childrenQuestionList.length;j++){
						if(childrenQuestionList[j].relationalIndicator=="TWINS"){
							qstHtml +='	<a href="javascript:;" data-type="'+childrenQuestionList[j].relationalIndicator+'" onclick="switchQues(this)" id="'+childrenQuestionList[j].queQuestionId+'">孪生题'+m+'</a>';
							m++;
						}
						if(childrenQuestionList[j].relationalIndicator=="EXTEND"){
							qstHtml +='	<a href="javascript:;" data-type="'+childrenQuestionList[j].relationalIndicator+'" onclick="switchQues(this)" id="'+childrenQuestionList[j].queQuestionId+'">衍生题'+n+'</a>';
							n++;
						}
					}
				}
			...
			quesContainer = '<div id="quesContainer" class="quesContainer" style="padding-top:25px;" align="center">'+noRecord+'</div>';
		}
	...
	function switchQues(obj){
    	...
    	// 添加切换子题同时切换编辑和删除绑定的queQuestionId
    	$(obj).parent().prev().children(".skyBlue").each(function(){
    		$(this).attr("id",queQuestionId);
    	});
		...
		$.get(url,{"queQuestionId":queQuestionId},function(data){
			if(data){
				...
				var qstObj = data;
				var qstHtml = "";
				if(qstObj.content == null ){
					qstObj.content = "";
				}
				qstHtml+='<div style="cursor:pointer;" onclick="showAnalytical(this)">'+qstObj.content+'</div>';
				if(qstObj.contentVideo && qstObj.contentVideo != '' &&  qstObj.contentVideo != null){
					qstHtml += '<p><a href="javascript:;" class="playVideoP" ><button class="btn playVideo" id="'+qstObj.contentVideo+'" onclick="playVideo(this)" >点击播放音视频</button><span id="watch_play0s"></span></a></p>';
				}
				var type = qstObj.type;

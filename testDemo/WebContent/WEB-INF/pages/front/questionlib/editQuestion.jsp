...
<body class="editQuestion">
	...
	<div class="editQues">
		...
		<form name="editForm" id="editForm" method="post" class="mainUl">
		    ...
		    <!-- 将要进行修改的习题的id作为隐藏域-->
		    <input type="hidden" value="${questionObj.queQuestionId}" name="currentQuestionId"/>
		    <!-- 将本题的motherId作为一个隐藏域-->
			...
			<ul class="commonUL  questionItem" data-qusetionIndex = '0'>
				<li><label class="text left">来源</label> 
					<span class="cont"> 
						<input id="Resource" type="text" style="width: 470px" name="source" value='${questionObj.source}'/>
					</span>
				</li>
				...
				<li><label class="text left"><span class="red">*</span>权限</label> 
					<span class="cont">
						<c:if test="${requestScope.userType!=null}">
					       ...
					       <label class="mr20"><input name="usableType" id="useSchool" value="SCHOOL" type="radio">&nbsp;&nbsp;仅供学校使用</label>
					       <label class="mr20"><input name="usableType" id="share" value="PUBLIC" type="radio">&nbsp;&nbsp;分享到平台</label>
					       <label class="mr20"><input name="usableType" id="own" value="PRIVATE" type="radio" checked="checked">&nbsp;&nbsp;仅自己使用</label>
					       </c:if>
					   </c:if>
					    <c:if test="${requestScope.userType!=null}">
					       <c:if test="${requestScope.userType=='SCHOOL_USR'}">
					       <label class="mr20"><input name="usableType" id="useSchool" value="SCHOOL" type="radio" checked="checked">&nbsp;&nbsp;仅供学校使用</label>
					       <label class="mr20"><input name="usableType" id="share" value="PUBLIC" type="radio">&nbsp;&nbsp;分享到平台</label>
					       </c:if>
					   </c:if>	
					    <c:if test="${requestScope.userType!=null}">
					       <c:if test="${requestScope.userType=='AREA_USR'}">
					       <label class="mr20"><input name="usableType" id="areas" value="AREA" type="radio" checked="checked">&nbsp;&nbsp;仅供辖区内使用</label>
					       <label class="mr20"><input name="usableType" id="share" value="PUBLIC" type="radio">&nbsp;&nbsp;分享到平台</label>
					       </c:if>
					   </c:if>	
					</span>
		 ...
		 <li class="submitBtn"  style="margin-left:450px; margin-top:30px;">
		 <a class="btn bigBtn saveAndSubmit mr20" href="javascript:;" id="submitBtn">提交保存</a> 
		 </li>
		</ul>
	</div>
	...
	$(function () {
		...
		bindDifficulty();
		 ...
		 var questionId = '${questionObj.queQuestionId}';
		 ...
		 var usableType='${questionObj.usableType}';
		 var relationalIndicator='${questionObj.relationalIndicator}';
		 var source='${questionObj.source}';
	     ...
	     //选中本题的权限SCHOOL  AREA  PRIVATE
	     if("PUBLIC"==usableType){
	    	$("#share").attr("checked","checked");
	     }else if("AREA"==usableType){
	    	$("#areas").attr("checked","checked");
	     }else if("SCHOOL"==usableType){
	    	$("#useSchool").attr("checked","checked");
	     }
								...
								for (var i = 0, len = trs.length; i < len; i++) {
									...
									for (var j = 0, l = cols.length; j < l; j++) {
										if (cols[j]) {
											...
											console.log($tr.find('.edit-pop-btn:eq(' + j + ') textarea')[0]);
											...
											console.log($tr.find('.edit-pop-btn:eq(' + j + ') textarea')[0].value);
										}
									}
								}
							}
		...
		$('.mainUl').on('click', '.addOpt', function () {	
			...
			}else if($(".optDiv li").size()==3){
				//隐藏
				//alert("llllllllll");
				$(".optDiv li div a").css("display","none");
			}
			...
			//alert('${requestScope.countSons}');
			var index = $('.questionSubItem').length;
			if(index >= (10-'${requestScope.countSons}')) {Win.alert("最多添加10题子题！"); return;}
		    ...
		    var cds = "uploadInfoBox_content"+qusetionIndex+"flag";
	        var sds="uploadInfoBox_resolve"+qusetionIndex+"flag";
  	     ...
  	     if("1"==answerTypeValue){//表示是独立答案
        	   ...
        	   // edt_content.sync();
        	    var formData = $("#editForm").serialize();
        	 	$.post("${root}/createquestionlib/updatequestion.do?semesterOpts="+semesterId+"&classlevelOpts="+classLvlId+"&disciplineOpts="+disciplineId, formData,
        				function(res) {
        					if(res=="yes"){
        						Win.alert("提交成功");
...
//选择不同的知识点事件(根据)
		 ...
		 function showEditKnowContent(){
			  ...
			  if(""!=rKnowIdStr || null!=rKnowIdStr && ""!=rKnowledgeIdStr || null!=rKnowledgeIdStr && ""!=rKnowledgeNameStr || null!=rKnowledgeNameStr){
				  ...
				  for(var i=0; i<rKnowIdStrArr.length; i++){
					  ...
					  if(""!=rKnowIdStrArr[i] && ""!=rKnowledgeIdStrArr[i] && ""!=rKnowledgeNameStrArr[i]){
					   ...
					   var connHtml = "<span class=\"smallBlock chapBlock itemDel\" style=\"height:28px;\" child ='"+rKnowledgeIdStrArr[i]+"'  id='"+rKnowIdStrArr[i]+"'>" + rKnowledgeNameStrArr[i] + "<a href=\"javascript:remoKnowledge('"+rKnowIdStrArr[i]+"')\"></a><input type=\"hidden\" name=\"k_input\" value='"+rKnowledgeIdStrArr[i]+"'/></span>";
							 $("#selectedKnowledges").append(connHtml);
					  }
				  }
			  }
		 ...
		 function showEditchapterContent(){//本题目关联的所有版本分册章节在页面进行显示
			   ...
			   if(""!=chapterSecNameStr || null!=chapterSecNameStr && ""!=chapSecVerStr || null!=chapSecVerStr && ""!=chapSecIdStr || null!=chapSecIdStr){
					  ...
					  for(var i=0; i<chapterSecNameStrArr.length; i++){
						  ...
						  if(""!=chapterSecNameStrArr[i] && ""!=chapSecVerStrArr[i] && ""!=chapSecIdStrArr[i]){
							  ...
							  var connHtml = "<span class=\"smallBlock chapBlock itemDel\" style=\"height:28px;\" child ='"+chapSecVerStrArr[i]+"' id='"+chapSecIdStrArr[i]+"'>"
								+ chapterSecNameStrArr[i] + "<a href=\"javascript:remo('"+chapSecIdStrArr[i]+"')\"></a><input type=\"hidden\" name=\"ch_input\" value='"+chapSecVerStrArr[i]+"'/></span>";
						$("#selectedChapters").append(connHtml);
					  }
				  }
			   }
	 ...
	 window["uploadSwf"+id].uploadEvent.add("onComplete",function(){
  			...
  			var myProgressBox = $class("progressBox",$id(elm))[0],
	  	 	...
	  	 	window["uploadSwf"+id].uploadEvent.add("onOpen",function(){
 		       ...
 		       if($(""+spanJudge+" ul li").size()>=1 || $(""+uId+" p").size()>=1){
	  	 			//alert("size="+$(""+spanJudge+" ul").size());
	  	 			Win.alert("请删除已有的音视频才可上传新的视频文件!");
					return;
	  	 		} 

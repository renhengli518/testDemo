...
$(document).ready(function(){
	//初始化设置为编辑真题试卷保存
	$("#saveOrEditType").val("edit");
	$("#scoreInput2").html(score);
	$(".fixedRight").hide();
	//绑定难易程度下拉框
	bindDifficulty();
		...
		function initClasslevel(){
			...
			$.post(root+"/commons/classLevelBySemId.do?semesterId=" + checkedOption, function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(data[i].baseClasslevelId == baseClasslevelId){
						html += "<option value="+data[i].baseClasslevelId+" selected='selected' >" + data[i].classlevelName + "</option>";
					}else{
	...
	//学科级联年级   
		function initSubject(){
			//获得当前选中的年级的id
			$.post(root+"/commons/subjbyclassid.do?classLevelId=" + baseClasslevelId, function(data) {
				var html = '<option value="0">请选择</option>';
				for (var i = 0; i < data.length; i++) {
					if(data[i].baseSubjectId == baseSubjectId){
						html += "<option value="+data[i].baseSubjectId+" selected='selected'>" + data[i].baseSubjectName + "</option>";
					} else {
		...
		function initVersion() {
			//获得当前选中学科id
			//获得当前选中的年级的id
			$.post(root+"/commons/versionbysuclaid.do?baseClassLevelId=" + baseClasslevelId
					+ "&baseSubjId=" + baseSubjectId, function(data) {
				var html = '<option value="0">请选择</option>';
				for (var i = 0; i < data.length; i++) {
			...
			//初始化知识点
			 //绑定首节知识点
	   	    ...
	   	    bindKnowledgeBySemesterAndDiscipline(semesterId,baseSubjectId);
	...
	$("#versionOption").change(
			function() {
				...
				$.post(root + "/commons/volumeversionbyverId.do?baseSubjId=" + subjOption
						+ "&baseClassLevId=" + classLevelOption + "&baseVersionId=" + versionOption,
						function(data) {
							var html = '<option value="0">请选择</option>';
							for (var i = 0; i < data.length; i++) {
								html += "<option value=" + data[i].baseVolumeId + ">"
									 + data[i].volumeName + "</option>";
							}
	...
	$("#volumeOption").change(
			function() {
				...
				$.post(root + "/commons/chaptervolumebycid.do?baseVersionId=" + versionOption
						+ "&baseSubjId=" + subjOption + "&baseClassLevId=" + classLevelOption
						+ "&baseVolumeId=" + volumeOption, function(data) {
					var html = '<option value="0">请选择</option>';
					for (var i = 0; i < data.length; i++) {
	...
	$("#chapterOption").change(
			function() {
				...
				$.post(root + "/commons/sectionchapbycid.do?baseVersionId=" + versionOption
						+ "&baseSubjId=" + subjOption + "&baseClassLevId=" + classLevelOption
						+ "&baseChapterId=" + chapterOption + "&baseVolumeId=" + volumeOption,
						function(data) {
							var html = '<option value="0">请选择</option>';
							for (var i = 0; i < data.length; i++) {
	...
	//初始化真题试卷习题列表
	$.get(root + "/schoolTest/getRealExamQuestions.do", 
		{
		"examId" : examId
		}, function(data) {
		if (data && data != null) {
			...
			data.forEach(function(value, index) {
				...
				var html = "";
				// 首先清除原来的习题，添加编辑后的习题
				html += '<div class="quesContainer">' + '<div class="quesDetail1" align="left" id="'
						+ value.examQuestionId + '">' + ' <p class="quesDesc">'
						+ '   <label>组卷次数：</label><span id="useCount" class="red">0</span>'
						+ '   <label>知识点：</label><span>' + value.endKonwledges + '</span>'
						+ '   <label>难易度：</label><span id="difficultyOpts">'
						+ switchDifficulty(value.difficulty) + '</span>'
						+ '   <label>更新时间：</label><span id="createTime">'
						+ now('y-m-d h:i:s', value.updateTime) + '</span>'
						+ '   <span class="fr editOrDel">'
						+ '     <span onclick="editQuestion(this)" id="' + value.examQuestionId
						+ '">编辑</span>' + '     <b class="c888">|</b>' + '     <span class="del" id="'
						+ value.examQuestionId + '" onclick="deleteQuestion(this,\'' + value.type
						+ '\')">删除</span>' + '   </span>' + ' </p>' + '   <div class="quesWrap show">'
						+ value.content;
...
$("#addExamQue").on("click",function(){
	...
	for (var i = 0; i < FillCountArr.length; i++) {// 题目的个数和填空题数组的大小一致(因为一道题一个隐藏域)
		...
		// alert("第"+(i+1)+"题:"+questionCount_1+"==="+answerTypeValue+"====index="+questionIndex);
		if (questionCount_1 == 0) {// 表示题干中未填充填空题
			...
			if ("2" == answerTypeValue) {// 组合答案
				...
				for (var k = 1; k < 5; k++) {
					...
					if (1 == k) {
						if (count < questionCount_1) {// 表示第一列未满列
							Win.alert("请按顺序将组合答案填全!");
							return;
						}
					}
					if (2 == k) {
	...
	// 由于编辑真题试卷设置了一些select框为disabled，无法传值，所以在提交的时候取消disabled属性，提交成功后再加上disabled属性
	$("#semesterOption").removeAttr("disabled");
	$("#classLevelOption").removeAttr("disabled");
	$("#subjOption").removeAttr("disabled");
	var html = "";
    //添加到qlist对象维护
	...
	$.post(root+"/schoolTest/createTempoparyQuestion.do", formData,
			function(data) {
				$("#semesterOption").attr("disabled","disabled");
				$("#classLevelOption").attr("disabled","disabled");
				$("#subjOption").attr("disabled","disabled");
				//首先清空之前的题目
				if(data && data!=null){
					var value = data;
						//用来添加设置分数页面的临时题目
					...
					html+='<div class="quesContainer">'
						+'<div class="quesDetail1" align="left" id="'+value.examQuestionId+'">'
						+' <p class="quesDesc">'
						+'   <label>组卷次数：</label><span id="useCount" class="red">0</span>'
						+'   <label>知识点：</label><span>'+value.endKonwledges+'</span>'
...
function getRealExamQuestions(value){
	...
	var html="";
	//首先清除原来的习题，添加编辑后的习题
	html+='<div class="quesDetail1" align="left" id="'+value.examQuestionId+'">'
		+' <p class="quesDesc">'
		+'   <label>组卷次数：</label><span id="useCount" class="red">0</span>'
		+'   <label>知识点：</label><span>'+value.endKonwledges+'</span>'
		+'   <label>难易度：</label><span id="difficultyOpts">'+switchDifficulty(value.difficulty)+'</span>'
...
function addSetScoreQuestion(data){
	...
	qstHtml += '	<label>难易度：</label><span class="red">'+switchDifficulty(qstObj.difficulty)+'</span>';
	qstHtml += '	<label>更新日期：</label><span>'+now('y-m-d h:i:s',qstObj.updateTime)+'</span>';
	qstHtml += '	<label>分值：</label><span class="c888 spanDate"><input id="score_'+qstObj.examQuestionId+'" type="text" class="littleInput" name="score"/>分</span>';
	qstHtml += "<button class='move move-up btn removeUp mr10'>上移</button>";
	qstHtml += "<button class='move move-up btn removeDown mr10'>下移</button>";
	qstHtml += '<button  class="fr  btn removeFromPaper removeQuestion '+qstObj.examQuestionId+'_btn" qid = '+qstObj.examQuestionId+'  qtype='+type+'>移出试卷</button></p>';
	qstHtml +='<div class="quesBro quesKinds">';
	...
	$("#score_"+qstObj.examQuestionId).val(qstObj.score);
}
